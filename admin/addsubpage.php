...
<?
	...
	if($_REQUEST["isPosted"] == 1)
	{
		...
		$pageObj->addNewSubPage($pageName, $page_title, $meta_keywords, $meta_tags, $content, $listorder, $parentPage);
		
		
		
		$_SESSION["er"] = _CFG_PAGE_ADD_SUCCESS;
		$con_id = mysql_insert_id();
		header("Location: editsubpagecontent.php?content=".$con_id."&from=m");
		exit;
	}
?>
...
?>
  ...
  ?><tr>
	...
	<form name="frmAddPage" action="addsubpage.php" method="post" onSubmit="return validate()">
						...
						<tr class="trblueOdd">
						  <td width="100%" height="26" class="titlebold"><div align="right">
							<table width="100%" border="0" cellpadding="8" cellspacing="0">
							  <tr>
								<td><span class="titlebold"><?=_CONTENT_PARENT_PAGE_TEXT?></span>&nbsp;<span class="title">
								<select name="parentPage" id="parebtPage" class="selectBox">
								<option value="">Select Parent Page</option>
								 <?php
										$parentPageArray = $pageObj->getAllParentPage();
										if(is_array($parentPageArray) && count($parentPageArray) > 0)
										{
											foreach($parentPageArray as $parentPage)
											{
												echo '<option value="' . $parentPage["contentid"] . '">' . $parentPage["contentname"] . '</option>';
											}
										}
								 ?>
								 </select>
...
function validate()
{
	...
	if(f.parentPage.value == "")
	{
		alert('<?=_CFG_PARENT_PAGE_ADD_ERROR?>');
		f.parentPage.focus();
		return false;
	}
