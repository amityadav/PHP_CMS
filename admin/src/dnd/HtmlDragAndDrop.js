	...
	onDragEnd: function(e){
		switch(e.dragStatus){
			...
			case "dropSuccess":
				...
				tempvarDrag = this.domNode.getAttribute('rowType').split('_');
				dragObjectId = tempvarDrag[1];
				dragObjectType = tempvarDrag[0];
...
dojo.lang.extend(dojo.dnd.HtmlDropTarget, {
	...
	_getNodeUnderMouse: function(e){
		...
		/*if(e.target.nodeName == "TD")
			if(e.target.getAttribute('isDroppable') == 0)
			{
				e.target.style.cursor = 'not-allowed';
				nodeUnderCursor = e.target;
			}else{
	...
	placeIndicator: function(e, dragObjects, boxIndex, before) {
		this.dropIndicator.style.cursor = 'not-allowed';
		with(this.dropIndicator.style){
			if (boxIndex < 0) {
				if (this.childBoxes.length) {
	...
	insert: function(e, refNode, position) {
		...
		/* By Amit Yadav / Vineet Malhotra */

		dragObjectId = '';
		dragObjectType = '';
		tempvarDrag = node.getAttribute('rowType').split('_');
		dragObjectId = tempvarDrag[1];
		dragObjectType = tempvarDrag[0];		
		dragObjectHasChild = tempvarDrag[2];
				

		dropObjectId = '';
		dropObjectType = '';
		tempVar = refNode.getAttribute('rowType').split('_');
		dropObjectId = tempVar[1];
		dropObjectType = tempVar[0];
		dropObjectHasChild = tempVar[2];
		
		

		
		if(dragObjectType == "parent")
		{
			if(dragObjectHasChild == 1)
			{				
				alert("Parent having child do not allow to transfer");
				this.onDragOut(e);
				return false;
			}
		}
		if(dragObjectType == "child" && dropObjectType == "child")
		{			
				alert("Child to child transfer not allowed!");
				//nodeUnderCursor.style.cursor = 'hand';
				this.onDragOut(e);
				return false;
		}

		if(dragObjectType == "parent" && dropObjectType == "child")
		{			
				alert("Parent to child transfer not allowed!");
				this.onDragOut(e);
				return false;
		}
		
		
		if(dropObjectId == dragObjectId)
		{			
				this.onDragOut(e);
				return false;
		}

		if(dropObjectType == "makeParent")
		{
				if (dragObjectType == "parent")
				{
					return false;
