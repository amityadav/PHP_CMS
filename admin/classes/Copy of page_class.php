		...
		function drawPagesListing()
		{
			...
			if($dbObject->num_rows > 0)
			{
				...
				for($k=0;$k<count($contentArray);$k++)
				{
					...
					$tableString .= '<span rowType="parent"  id="dragDrop' . $pageId . '"><table width="100%" border="0" cellpadding="4" cellspacing="0" style="align="center"><tr valign="top" class="' . $class . '">';

					if($rms_status=="on")
					{
						$tableString .= '<td width="15%" id="isActive' . $pageId . '"><font color="red">' . showStatus($contentArray[$k]["isActive"]) . '</font></td>
								<td width="5%"><input type="checkbox" id="checkall_' . $this->colorCounter . '"  name="checkall_' . $this->colorCounter . '" value="' . $pageId . '" ></td>';
					}
					
					$this->colorCounter++;
					$tableString .= '<td width="40%"><font color="red"><b>' . $contentArray[$k]["contentname"] . '</b></font></td>';

					if($rms_status=="on")
					{
						$tableString .= '<td width="15%">
						...
						$tableString .= '</td><td width="50%" class="<?=$class?>">';
					...
					$tableString .= '<td width="15%"><a href = "editcontent.php?content=' . $pageId . '&from=m" style="text-decoration:none" class="' . $class . '" title="' . _CFG_TITLE_EDIT_PAGE . '"><img src="images/b_edit.png" border="0"></a></td>';

					if($rms_status=="on")
					{
					$tableString .= '<td width="15%"><a href="javascript:void(0);"onClick="deletePage(\'' . $pageId . '\', \'1\')" title="' . _CFG_TITLE_DELETE_PAGE . '"><img src="images/trash.gif" border="0" alt="'. _CFG_TITLE_DELETE_PAGE .'"></a></td>';
					 }

					$tableString .= '<td width="10%"><a href ="../index.php?_p_=' . $contentArray[$k]["contentid"] . '" class="' . $class . '" style="text-decoration:none" target="_blank" title="' . _CFG_PREVIEW . '"><img src="images/preview.gif" border="0" alt="'. _CFG_PREVIEW .'"></a></td></tr></table></span>';
		...
		function drawSubPageListing($parentId)
		{
			...
			if($dbObject->num_rows > 0)
			{
				...
				for($amt=0;$amt<count($subContentArray);$amt++)
				{
					...
					$subPageTableString .= '<span  rowType="child" id="dragDrop' . $subPageId . '"><table width="100%" border="0" cellpadding="4" cellspacing="0" style="align="center"><tr valign="top" class="' . $class . '">';

					if($rms_status=="on")
					{
						$subPageTableString .= '<td width="15%" id="isActive' . $subPageId . '">' . showStatus($subContentArray[$amt]["isActive"]) . '</td><td width="5%"><input type="checkbox" id="checkall_' . $this->colorCounter . '"  name="checkall_' . $this->colorCounter . '" value="' . $subPageId . '" ></td>';
					}
					
					$this->colorCounter++;

					$subPageTableString .= '<td width="40%" style="margin-left:10px">&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/right.jpg" border="0">&nbsp;&nbsp;' . $subContentArray[$amt]["contentname"] . '</td>';

					if($rms_status == "on")
					{
						$subPageTableString .= '<td width="15%">
						<table width="40%" border="0">
						<tr>
						<td width="50%" class="' . $class . '">';
						...
						$subPageTableString .= '</td><td width="50%" class="<?=$class?>">';
					...
					$subPageTableString .= '<td width="15%"><a href = "editsubpagecontent.php?content=' . $subPageId . '&from=m" style="text-decoration:none" class="' . $class . '" title="' . _CFG_TITLE_EDIT_PAGE . '"><img src="images/b_edit.png" border="0"></a></td>';

					if($rms_status == "on")
					{
						$subPageTableString .= '<td width="15%"><a href="javascript:void(0);"onClick="deletePage(\'' . $subPageId . '\', \'0\')" title="' . _CFG_TITLE_DELETE_PAGE . '"><img src="images/trash.gif" border="0" alt="'. _CFG_TITLE_DELETE_PAGE .'"></a></td>';
					 }

					$subPageTableString .= '<td width="10%"><a href ="../index.php?_p_=' . $subContentArray[$amt]["contentid"] . '" class="' . $class . '" style="text-decoration:none" target="_blank" title="' . _CFG_PREVIEW . '"><img src="images/preview.gif" border="0" alt="'. _CFG_PREVIEW .'"></a></td></tr></table></span>';
				}
